;(function () {
  ...
  function usersSeting() {
    ...
    var setingData = [
      {
        tag: "bilibiliHelper",
        name: "B站使用助手(视频下载可P、浏览记录、一件三联)",
        checked: bilibiliHelper,
      },
      {
        tag: "zhihuHelper",
        name: "知乎使用助手(内容标识、问答显示优化、视频下载等)",
        checked: zhihuHelper,
      },
      {
        tag: "superVideoHelper",
        name: "全网VIP视频解析(支持爱奇艺、腾讯视频、B站番剧等)",
        checked: superVideoHelper,
      },
      {
        tag: "superMusicHelper",
        name: "全网VIP音乐解析(支持网易云音乐、QQ音乐等)",
        checked: superMusicHelper,
      },
      {
        tag: "abroadVideoHelper",
        name: "国外视频解析下载(支持油管、Facebook等)",
        checked: abroadVideoHelper,
      },
      {
        tag: "wangpanSearchEnginesHelper",
        name: "网盘搜索引擎破解无限下载",
        checked: wangpanSearchEnginesHelper,
      },
      {
        tag: "searchEnginesNavigation",
        name: "搜索引擎资源导航(适配百度、360、搜狗、必应等)",
        checked: searchEnginesNavigation,
      },
    ]
    var content = ""
  ...
  function superVideoHelper(originalInterfaceList) {
      ...
      if (!host.startsWith("m.")) {
        ...
        for (var i = 0; i < urls.length; i++) {
          ...
          if (window.location.host.indexOf("bilibili.com") == -1) {
            ...
            if (
              window.location.href.indexOf("www.bilibili.com/bangumi/play") !=
              -1
            ) {
              //是B站只有番剧才开启VIP解析
              result = true
    ...
    this.analysisCustomInterface = function () {
      //自定义接口解析
      var customInterface = commonFunctionObject.GMgetValue(
        this.customInterfaceKey,
        ""
      )
      if (!!customInterface) {
        try {
          ...
          for (var i = 0; i < analysisArray.length; i++) {
            ...
            if (!!onePiece && onePiece.indexOf(",") != -1) {
              ...
              if (
                onePieceArray.length == 2 &&
                !!onePieceArray[0] &&
                /(http|https):\/\/\S*/.test(onePieceArray[1])
              ) {
          ...
          this.originalInterfaceList = customizeInterfaceList.concat(
            this.originalInterfaceList
          )
        } catch (e) {}
      }
    }
    this.getQuicklyInterfaceIndex = function () {
      var quicklyInterfaceIndexString =
        commonFunctionObject.GMgetValue(this.quicklyInterfaceKey, "") + ""
    ...
    this.addHtmlElements = function () {
      ...
      var cssMould =
        ...
        ` .img_box` +
        this.elementId +
        `{width:24px; height:32px;line-height:32px;text-align:center;background-color:` +
        color +
        `;}
							#vip_movie_box` +
        this.elementId +
    ...
    this.runEvent = function () {
      ...
      var that = this
      $(
        "#vip_movie_box" + this.elementId + " >.plugin_inner_" + this.elementId
      ).on("mouseover", () => {
      ...
      $("#img_box_jump_6667897iio").on("click", function () {
        commonFunctionObject.GMopenInTab(
          "https://laisoyiba.com/mov/s/?sv=3&url=" + window.location.href,
          false
        )
      })
      ...
      movie_box.mousedown(function (e) {
        if (e.which == 3) {
          ...
          $(document).mousemove(function (e) {
            ...
            movie_box.css("top", y)
            commonFunctionObject.GMsetValue(
              "Position_" + window.location.host,
              { left: x, top: y }
            )
          })
    ...
    this.showSetingDialog = function () {
      ...
      popup.dialog({
        ...
        content: content,
        onContentReady: function ($that) {
          var $saveCustomInterfaceBtn = $that.dialogContent.querySelector(
            ".save-custom-interface-btn"
          )
          $saveCustomInterfaceBtn.addEventListener("click", function () {
            var $customInterfaceTextarea = $that.dialogContent.querySelector(
              ".custom-interface-textarea"
            )
  ...
  function superVideoHelperMobile(originalInterfaceList) {
    ...
    this.runEvent = function () {
      ...
      $("#start_analysis_outer_" + self.elementId).attr(
        "href",
        playObject.url + window.location.href
      )
      $("#start_analysis_outer_" + self.elementId).on("click", function (e) {
        if (
          commonFunctionObject.GMgetValue(
            "copyright_video_remind_mobile_outer",
            null
          ) === "true"
        ) {
        } else {
    ...
    this.downloadVideoCoin = function () {
      function innerClassDownloadVideoCoin() {
        ...
        this.showModals = function (pageHtml) {
          ...
          $("body").on(
            "click",
            ".modal-body-" + self.elementId + " >.page-wrap >.board-item",
            function () {
          ...
          $("body").on(
            "click",
            ".modal-body-" + self.elementId + " >.modal-btn-wrap >span",
            function () {
              self.hideModals()
            }
          )
        ...
        this.createElementHtml = function () {
          ...
          setTimeout(() => {
            ...
            const playerInterval = setInterval(() => {
              ...
              if (
                $("#download_s_" + randomNumber).length == 0 &&
              ...
              .then((resule) => {
                if (resule.status === "success") {
                  ...
                  for (var i = 0; i < pages.length; i++) {
                    var title =
                      "【P" + pages[i].page + "】" + pages[i].part + ""
                    itemHtml +=
                      "<span class='board-item' data-aid='" +
                      aid +
                      "' data-cid='" +
                      pages[i].cid +
                      "' title='" +
    ...
    this.recordView = function () {
      ...
      function innerClassRecordView() {
        ...
        this.searchPageRemindHtml = function ($ele, top = 8, right = 8) {
          ...
          $ele.append(
            "<div style='z-index: 99999999;position:absolute; top:" +
              top +
              "px; right:" +
              right +
        ...
        this.searchPageRemind = function () {
          ...
          setInterval(function () {
            let cacheText = commonFunctionObject.GMgetValue(
              $that.localCacheName
            )
            cacheText = !cacheText ? "" : cacheText
    ...
    this.start = function () {
      ...
      setInterval(function () {
        ...
        if (host.indexOf("youtube.com") != -1) {
          if (href.indexOf("youtube.com/watch") != -1) {
            ...
            var html =
              '<div id="' +
              eleId +
              '" style="width:25px;padding:10px 0px;text-align:center;background-color:#E5212E;position:fixed;top:250px;left:0px;color:#FFF;font-size:0px;cursor:pointer;margin:0px auto;text-align:center;z-index:9999999;">' +
              '<img src="' +
              iconVideo +
              '" style="width:20px;">' +
              "</div>"
            ...
            $("body").on("click", "#" + eleId, function () {
              ...
              var videourl =
                "https://www.ytdownfk.com/search?url=" + location_url
        ...
        if (host.indexOf("facebook.com") != -1) {
          if (
            href.indexOf("facebook.com/watch") != -1 ||
            ...
            $("body").on("click", "#" + eleId, function () {
              var location_url = window.location.href
              commonFunctionObject.GMsetValue("facebook_downloader_obj", {
                facebook_url: location_url,
              })
              commonFunctionObject.GMopenInTab(
                "https://yt1s.com/facebook-downloader",
                false
              )
            })
          } else {
      ...
      if (window.location.href.indexOf("yt1s.com/facebook-downloader") != -1) {
        //facebook下载
        var facebookObject = commonFunctionObject.GMgetValue(
          "facebook_downloader_obj"
        )
        if (!!facebookObject) {
  ...
  function queryCoupon() {
    ...
    this.getPlatform = function () {
      ...
      if (host.indexOf("detail.tmall") != -1) {
        ...
        host.indexOf("jd.com") != -1 ||
        host.indexOf("npcitem.jd.hk") != -1
      ) {
        platform = "jd"
      }
      return platform
    ...
    this.runAliDeceptionSpm = function () {
      if (
        window.location.host.indexOf("aliyun.com") != -1 ||
        window.location.host.indexOf("taobao.com") != -1 ||
        window.location.host.indexOf("tmall.com") != -1
      ) {
        this.randomSpmValue()
        setInterval(() => {
          this.randomSpmValue()
    ...
    this.createCouponHtml = function (platform, goodsId, goodsName) {
      ...
      var goodsCouponUrl =
        "http://tt.shuqiandiqiu.com/api/coupon/discover?no=5&v=1.0.2&pl=" +
        platform +
        "&id=" +
        goodsId +
        "&qu=" +
        goodsName
      var goodsPrivateUrl =
        "http://tt.shuqiandiqiu.com/api/private/change/coupon?no=5&v=1.0.2&platform=" +
        platform +
        "&id="
        ...
        if (resutData.result === "success" && !!resutData.json) {
          ...
          if ($llkk.length != 0) {
            ...
            couponElementA.unbind("click").bind("click", () => {
              ...
              if (!!couponId) {
                this.request("GET", goodsPrivateUrl + couponId, null).then(
                  (resutData2) => {
  ...
  function superMusicHelper() {
    ...
    this.getPlayUrl = function () {
      ...
      if (currentUrl.match(/music\.163\./)) {
        //网易云音乐
        if (
          currentUrl.match(/^https?:\/\/music\.163\.com\/#\/(?:song|dj)\?id/)
        ) {
          playUrl =
            "https://music.liuzhijin.cn/?url=" + encodeURIComponent(currentUrl)
        } else if (
          currentUrl.match(/^https?:\/\/y\.music\.163\.com\/m\/(?:song|dj)\?id/)
        ) {
          playUrl =
            "https://music.liuzhijin.cn/?url=" +
            encodeURIComponent(
        ...
        if (currentUrl.indexOf("#")) currentUrl = currentUrl.split("#")[0]
        var musicMatch = currentUrl.match(
          /^https?:\/\/y\.qq\.com\/n\/ryqq\/songDetail\/(\S*)/
        )
        if (musicMatch) {
          playUrl =
            "https://music.liuzhijin.cn/?id=" + musicMatch[1] + "&type=qq"
        }
        var musicMatch2 = currentUrl.match(
          /^https?:\/\/y\.qq\.com\/n\/yqq\/song\/(\S*).html/
        )
        if (musicMatch2) {
          playUrl =
            "https://music.liuzhijin.cn/?id=" + musicMatch2[1] + "&type=qq"
        }
        ...
        if (musicMatch) {
          playUrl =
            "https://music.liuzhijin.cn/?id=" + musicMatch[1] + "&type=kugou"
        }
        ...
        if (musicMatch) {
          playUrl =
            "https://music.liuzhijin.cn/?id=" + musicMatch[1] + "&type=kuwo"
        }
        ...
        if (currentUrl.indexOf("#")) currentUrl = currentUrl.split("#")[0]
        var musicMatch = currentUrl.match(
          /^https?:\/\/www\.lizhi\.fm\/(\d*)\/(\d*)/
        )
        if (musicMatch) {
          playUrl =
            "https://music.liuzhijin.cn/?id=" + musicMatch[2] + "&type=lizhi"
        }
        ...
        if (currentUrl.indexOf("#")) currentUrl = currentUrl.split("#")[0]
        var musicMatch = currentUrl.match(
          /^https?:\/\/music\.migu\.cn\/v3\/music\/song\/(\S*)/
        )
        if (musicMatch) {
          playUrl =
            "https://music.liuzhijin.cn/?id=" + musicMatch[1] + "&type=migu"
        }
      }
      return playUrl
    }
    ...
    this.generateHtml = function () {
      ...
      html +=
        "<div class='plugin_item jump_analysis_website' title='点我VIP音乐破解，免客户端下载！'><img src='" +
        vipImgBase64 +
        "'></div>"
      html += "</div>"
      $("body").append(html)

      $("#plugin_kiwi_analysis_vip_music_box_" + this.eleId + "").on(
        "click",
        function () {
    ...
    this.operation = function () {
      ...
      setInterval(function () {
        var playUrl = $that.getPlayUrl()
        var $vipMusicBox = $(
          "#plugin_kiwi_analysis_vip_music_box_" + $that.eleId + ""
        )
        if (!!playUrl) {
    ...
    this.start = function () {
      ...
      if (host === "www.laisoyixia.com" || host === "www.xiaozhukuaipan.com") {
        ...
        if (!!downloadurl) {
          ...
          $downloadBtn.after(
            "<div style='padding:15px;background-color:#eee;margin-top:15px;'>插件提取所得：<a target='_blank' href='" +
              wangpanUrl +
              "'>" +
              wangpanUrl +
              "</a></div>"
  ...
  function searchEnginesNavigation() {
    ...
    this.getNavigationData = function (element, elementInput) {
      const navigationData = [
        {
          ...
          list: [
            {
              name: "书签搜索",
              url: "https://www.bookmarkearth.com/s/search?q=@@&currentPage=1",
            },
            {
              name: "网盘搜索",
              url: "https://www.xiaozhukuaipan.com/s/search?q=@@&currentPage=1",
            },
            {
              name: "财经搜索",
              url: "https://www.shaduizi.com/s/search?q=@@&currentPage=1",
            },
            ...
            {
              name: "抖音搜索",
              url: "https://www.douyin.com/search/@@?aid=0a9fc74b-01e8-4fb0-9509-307c5c07fda1&publish_time=0&sort_type=0&source=normal_search&type=general",
            },
            {
              name: "搜狗|公众号",
            ...
            {
              name: "搜狗|知乎",
              url: "https://www.sogou.com/sogou?pid=sogou-wsse-ff111e4a5406ed40&insite=zhihu.com&ie=utf8&p=73351201&query=@@&ie=utf8&p=73351201&query=@@",
            },
            ...
            {
              name: "PPT搜索",
              url: "https://www.chuliansheji.com/s/search?q=@@&currentPage=1&c=1",
            },
            {
              name: "icon搜索",
            ...
            {
              name: "今天买啥",
              url: "https://www.whatbuytoday.com/s/search?q=@@&currentPage=1",
            },
          ],
        },
        {
      ...
      const $that = this
      const cacheNavigationData = commonFunctionObject.GMgetValue(
      ...
      commonFunctionObject
        .request(
          "get",
          "http://api.staticj.top/script/api/get/navigation_json_url",
          null
        )
        .then((resultData) => {
          let dataJson = JSON.parse(resultData.data)
            ...
            commonFunctionObject
              ...
              .then((resultData2) => {
                ...
                if (
                  !cacheNavigationData ||
                  (!!cacheNavigationData &&
                    serverNavigationData.length !=
                      JSON.stringify(cacheNavigationData).length)
                ) {
                  commonFunctionObject.GMsetValue(
                    "navigation_data_cache",
                    JSON.parse(serverNavigationData)
                  )
                }
              })
              .catch(() => {})
    ...
    this.createCss = function (elementNum) {
      ...
      if ($("#plugin_css_style_dddsoo").length == 0) {
        $("body").prepend(
          "<style id='plugin_css_style_dddsoo'>" + innnerCss + "</style>"
        )
      }
    }
        ...
        if (isComplate) {
          ...
          if ($element.length != 0 && $box.length == 0) {
            ...
            for (var i = 0; i < navigationData.length; i++) {
              ...
              html +=
                "<div class='title'><b>" + navigationData[i].name + "</b></div>"
            ...
            $("#dsdsd99mmmjj7760011 a").on("click", function (e) {
              commonFunctionObject.GMopenInTab(
                $(this).data("url").replace("@@", $(elementInput).val()),
                false
              )
              e.preventDefault()
            })
    ...
    this.show = function () {
      ...
      if (
        ...
        (host === "www.sogou.com" &&
          href.indexOf("www.sogou.com/web?query") != -1) ||
        (host === "cn.bing.com" && href.indexOf("cn.bing.com/search") != -1) ||
        (host === "so.toutiao.com" &&
          href.indexOf("so.toutiao.com/search") != -1)
      ) {
        ...
        if (currentSearchEnginesData != null) {
          self.getNavigationData(
            currentSearchEnginesData.element,
            currentSearchEnginesData.elementInput
          )
        }
      }
    }
  ...
  function zhihuHelper() {
    ...
    this.markQuestionDate = function () {
      ...
      function addDateQuestion() {
        ...
        if (!!title) {
          var dateCreated = title.querySelector(
            "[itemprop~=dateCreated][content]"
          ).content
          var dateModified = title.querySelector(
      ...
      function addTimeAnswerItems() {
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          try {
            var dateCreated = item.querySelector(
              "[itemprop~=dateCreated][content]"
            ).content
            var dateModified = item.querySelector(
              "[itemprop~=dateModified][content]"
            ).content
            var createDate = DateFormat(
              new Date(dateCreated),
            ...
            timeDiv.style.cssText =
              "color:#6f6f6f;font-size:13px;display:block;padding:5px 0px;"
    ...
    this.showQuestionAuthor = function () {
      ...
      if (document.querySelector(".BrandQuestionSymbol, .QuestionAuthor"))
        return
    ...
    this.downloadVideo = function () {
      ...
      document.addEventListener("DOMNodeInserted", (e) => {
        ...
        var playBar = e.relatedNode.querySelector(
          ":scope > div:last-child > div:first-child > div:nth-of-type(2)"
        )
        if (!playBar || playBar.querySelector("#" + downloadElementId)) return
        ...
        downloadBut.setAttribute("id", downloadElementId)
  ...
  try {
    if (functionController.superVideoHelper) {
      const superVideoHelperObject = new superVideoHelper(
        newOriginalInterfaceList
      )
      if (superVideoHelperObject.isRun()) {
        if (
          commonFunctionObject.GMgetValue("copyright_video_remind", null) ===
          "true"
        ) {
